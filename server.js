...
app.get("/:room", (req, res) => {
  if (req.session.loggedIn == true) {
    const room = roomModel.find((room) => room.roomSlug === req.params.room);
    res.render("room", {
      layout: "rooms",
      rooms: roomModel,
...
io.on("connection", (socket) => {
  ...
  socket.on("send-chat-message", (roomSlug, message) => {
    ...
    socket.to(roomSlug).emit("chat-message", {
      username: rooms[roomSlug].users[socket.id],
      message: message,
    });
    console.log(rooms);
  });
  socket.on("disconnect", () => {
    getUserRooms(socket).forEach((roomSlug) => {
      socket
        .to(roomSlug)
        .emit("user-disconnected", rooms[roomSlug].users[socket.id]);
